...
All SQLAlchemy models are intentionally kept in this single file.
"""
...
class ViewerTypeEnum(str, enum.Enum):
    CUSTOMER = "customer"
    ANONYMOUS = "anonymous"


# =============================================================
# SERVICE VERSION REVIEW ENUMS
# =============================================================
...
class Customer(Base):
    ...
    service_views: Mapped[List["ServiceView"]] = relationship(
        back_populates="customer"
    )
...
class Vendor(Base):
    ...
    subscriptions: Mapped[List["VendorSubscription"]] = relationship(
        back_populates="vendor",
        cascade="all, delete-orphan",
    )
    leads: Mapped[List["Lead"]] = relationship(
        back_populates="vendor",
        foreign_keys="Lead.vendor_id",
    )
    bookings: Mapped[List["Booking"]] = relationship(
        back_populates="vendor"
    )
    kyc: Mapped[Optional["VendorKYC"]] = relationship(
        back_populates="vendor",
        uselist=False,
    )
...
class Service(Base):
    ...
    current_live_version_id: Mapped[Optional[int]] = mapped_column(
        BigInteger,
        ForeignKey(
            "service_versions.id",
            use_alter=True,
            name="fk_service_live_version",
        ),
        nullable=True,
    )
    ...
    rating_summary: Mapped[Optional["ServiceRatingSummary"]] = relationship(
        back_populates="service",
        uselist=False,
        cascade="all, delete-orphan",
    )

    wishlisted_by: Mapped[List["Favorite"]] = relationship(
        back_populates="service",
        cascade="all, delete-orphan",
    )
    ...
    views: Mapped[List["ServiceView"]] = relationship(
        back_populates="service",
        cascade="all, delete-orphan",
...
class ServiceVersion(Base):
    ...
    audit_log: Mapped[List["ServiceVersionAudit"]] = relationship(
        back_populates="version",
        cascade="all, delete-orphan",
    )

    # Service-type detail relationships
    venue_detail: Mapped[Optional["Venue"]] = relationship(
        back_populates="version",
        uselist=False,
        cascade="all, delete-orphan",
    )

    catering_detail: Mapped[Optional["CateringDetail"]] = relationship(
        back_populates="version",
        uselist=False,
        cascade="all, delete-orphan",
    )

    dj_detail: Mapped[Optional["DjDetail"]] = relationship(
        back_populates="version",
        uselist=False,
        cascade="all, delete-orphan",
    )

    photography_detail: Mapped[Optional["PhotographyDetail"]] = relationship(
        back_populates="version",
        uselist=False,
        cascade="all, delete-orphan",
    )

    event_management_detail: Mapped[
        Optional["EventManagementDetail"]
    ] = relationship(
        back_populates="version",
        uselist=False,
        cascade="all, delete-orphan",
    ...
    makeup_artist_detail: Mapped[
        Optional["MakeupArtistDetail"]
    ] = relationship(
        back_populates="version",
        uselist=False,
        cascade="all, delete-orphan",
    ...
    # Granular admin review workflow
    review_items: Mapped[List["ServiceVersionReviewItem"]] = relationship(
    ...
    __table_args__ = (
        UniqueConstraint(
            "service_id",
            "version_number",
            name="uq_service_version_number",
        ),
...
class ServiceVersionAudit(Base):
    ...
    service_version_id: Mapped[int] = mapped_column(
        BigInteger,
    ...
    performed_by_type: Mapped[Optional[str]] = mapped_column(
        String(20)
    )
    ...
    version: Mapped["ServiceVersion"] = relationship(
...
# =============================================================
# SECTION 3B: SERVICE VERSION REVIEW ITEMS
# =============================================================

class ServiceVersionReviewItem(Base):
    """
    Tracks the admin review status of individual sections of a service version.

    A version gets one review item per section. This allows an admin to
    approve some sections while requesting changes to others, for example:
    media, pricing variants, or service details.
    """
    ...
    section: Mapped[ReviewSection] = mapped_column(
        Enum(ReviewSection, name="review_section"),
        nullable=False,
    )
    ...
    # Relationships
    service_version: Mapped["ServiceVersion"] = relationship(
        back_populates="review_items"
    )

    reviewer: Mapped[Optional["Admin"]] = relationship(
    ...
    __table_args__ = (
        UniqueConstraint(
            "service_version_id",
            "section",
            name="uq_review_item_version_section",
        ),
...
class ServiceVariant(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "service_version_id",
            "variant_name",
            name="uq_variant_version_name",
        ),
        Index("idx_variant_version", "service_version_id"),
        Index("idx_variant_pricing_type", "pricing_type"),
        Index(
            "idx_variant_pricing_json",
            "pricing",
            postgresql_using="gin",
            postgresql_ops={"pricing": "jsonb_path_ops"},
        ),
    )
...
class Venue(Base):
    ...
    parking_capacity: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )

    venue_policies: Mapped[dict] = mapped_column(
        JSONB,
        nullable=False,
...
class CateringDetail(Base):
    ...
    meal_types: Mapped[Optional[list]] = mapped_column(
        ARRAY(String(20)),
        server_default=text("ARRAY['lunch']"),
    )

    veg_price_per_head: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2)
    )

    non_veg_price_per_head: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2)
    )
    ...
    service_styles: Mapped[list] = mapped_column(
        JSONB,
        nullable=False,
        server_default=text("'[]'::jsonb"),
    )

    staff_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )

    crockery_cutlery_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )

    tasting_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    ...
    travel_cost_per_km: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2)
    )
    ...
    price_includes_tax: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )

    special_diets_supported: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )

    customizable_menu: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    ...
    version: Mapped["ServiceVersion"] = relationship(
        back_populates="catering_detail"
    )

    __table_args__ = (
        CheckConstraint(
            "veg_price_per_head >= 0",
            name="chk_veg_price",
        ),
        CheckConstraint(
            "non_veg_price_per_head >= 0",
            name="chk_non_veg_price",
        ),
        CheckConstraint(
            "min_order > 0",
            name="chk_min_order",
        ...
        CheckConstraint(
            "max_order IS NULL OR max_order >= min_order",
            name="chk_catering_max_order",
        ),
        Index("idx_catering_version", "service_version_id"),
        Index(
            "idx_catering_cuisine",
            "cuisine_types",
            postgresql_using="gin",
        ),
        Index(
...
class DjDetail(Base):
    ...
    genres_supported: Mapped[list] = mapped_column(
        JSONB,
        nullable=False,
        server_default=text("'[]'::jsonb"),
    )
    ...
    event_types_supported: Mapped[Optional[list]] = mapped_column(
        ARRAY(Enum(DjEventTypeEnum, name="dj_event_type_enum"))
    )

    performance_duration_hours: Mapped[Decimal] = mapped_column(
        Numeric(5, 2),
        nullable=False,
    )

    overtime_rate_per_hour: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2)
    )

    equipments_provided: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )

    sound_system_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    lighting_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    smoke_machine_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    led_wall_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    mc_host_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )

    crowd_interaction_level: Mapped[Optional[str]] = mapped_column(
        String(20)
    )

    setup_time_minutes: Mapped[Optional[int]] = mapped_column(Integer)
    teardown_time_minutes: Mapped[Optional[int]] = mapped_column(Integer)
    power_requirement_kw: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(5, 2)
    )

    backup_power_required: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    ...
    outdoor_supported: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    late_night_allowed: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    sound_license_required: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    custom_playlist_allowed: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    playlist_link_supported: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    ...
    total_events_performed: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )
    ...
    version: Mapped["ServiceVersion"] = relationship(
        back_populates="dj_detail"
    )

    __table_args__ = (
        CheckConstraint(
            "performance_duration_hours > 0",
            name="chk_dj_duration",
        ),
        CheckConstraint(
            "crowd_interaction_level IS NULL OR "
            "crowd_interaction_level IN ('low','medium','high')",
...
class PhotographyDetail(Base):
    ...
    photography_types: Mapped[list] = mapped_column(
        JSONB,
        nullable=False,
        server_default=text("'[]'::jsonb"),
    )

    videography_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )

    drone_shoot_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    ...
    video_delivery_duration_minutes: Mapped[Optional[int]] = mapped_column(
        Integer
    )

    edited_photos_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )

    raw_photos_provided: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )

    album_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    ...
    coverage_hours: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(5, 2)
    )
    ...
    second_shooter_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )

    editing_styles: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )
    ...
    total_events_covered: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )
    ...
    version: Mapped["ServiceVersion"] = relationship(
        back_populates="photography_detail"
    )
...
class EventManagementDetail(Base):
    ...
    event_types_supported: Mapped[list] = mapped_column(
        JSONB,
        nullable=False,
        server_default=text("'[]'::jsonb"),
    )

    services_offered: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )

    themes_supported: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )
    ...
    on_site_managers: Mapped[int] = mapped_column(
        Integer,
        server_default=text("1"),
    )

    decoration_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    catering_management: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    entertainment_management: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    vendor_coordination_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    guest_management_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    logistics_management_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    ...
    teardown_time_hours: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(5, 2)
    )
    ...
    serviceable_cities: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )
    ...
    total_events_managed: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )
    ...
    version: Mapped["ServiceVersion"] = relationship(
        back_populates="event_management_detail"
    )

    __table_args__ = (
        CheckConstraint(
            "max_budget IS NULL OR min_budget IS NULL "
            "OR max_budget >= min_budget",
            name="chk_event_budget_valid",
        ),
...
class MakeupArtistDetail(Base):
    ...
    makeup_types: Mapped[list] = mapped_column(
        JSONB,
        nullable=False,
        server_default=text("'[]'::jsonb"),
    )
    ...
    brands_used: Mapped[list] = mapped_column(
        JSONB,
        server_default=text("'[]'::jsonb"),
    )

    premium_products_used: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    ...
    assistants_count: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )

    service_duration_minutes: Mapped[Optional[int]] = mapped_column(
        Integer
    )

    travel_to_client: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    ...
    hairstyling_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("true"),
    )
    draping_included: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    nail_art_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    trial_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )
    paid_trial_available: Mapped[bool] = mapped_column(
        Boolean,
        server_default=text("false"),
    )

    trial_cost: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2)
    )
    ...
    total_clients_served: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )

    total_bridal_bookings: Mapped[int] = mapped_column(
        Integer,
        server_default=text("0"),
    )
    ...
    version: Mapped["ServiceVersion"] = relationship(
        back_populates="makeup_artist_detail"
    )
...
class Review(Base):
    ...
    food_beverage_rating: Mapped[Optional[int]] = mapped_column(
        SmallInteger
    )
    service_quality_rating: Mapped[Optional[int]] = mapped_column(
        SmallInteger
    )
    ambiance_rating: Mapped[Optional[int]] = mapped_column(
        SmallInteger
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "food_beverage_rating IS NULL OR "
            "food_beverage_rating BETWEEN 1 AND 5",
            name="chk_fb_rating",
        ),
        CheckConstraint(
            "service_quality_rating IS NULL OR "
            "service_quality_rating BETWEEN 1 AND 5",
            name="chk_sq_rating",
        ),
        CheckConstraint(
            "ambiance_rating IS NULL OR "
            "ambiance_rating BETWEEN 1 AND 5",
            name="chk_ambiance_rating",
        ),
        CheckConstraint(
            "value_for_money_rating IS NULL OR "
...
class Favorite(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "wishlist_id",
            "service_id",
            name="uq_favorite_wishlist_service",
        ),
    )
...
class Lead(Base):
    ...
    service_type: Mapped[Optional[ServiceTypeEnum]] = mapped_column(
        Enum(ServiceTypeEnum, name="service_type_enum")
    )
    ...
    customer_status: Mapped[CustomerLeadStatusEnum] = mapped_column(
        Enum(
            CustomerLeadStatusEnum,
            name="customer_lead_status",
        ),
        nullable=False,
...
class UnlockUsage(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "vendor_id",
            "usage_date",
            name="uq_unlock_usage_vendor_date",
        ),
        Index(
            "idx_unlock_usage_vendor",
            "vendor_id",
            "usage_date",
        ),
    )
...
class Notification(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "recipient_type IN ('customer', 'vendor', 'admin')",
...
# =============================================================
# SECTION 16: SERVICE VIEWS / ANALYTICS
# =============================================================

class ServiceView(Base):
    ...
    # Persistent browser identifier for anonymous visitors
    visitor_id: Mapped[Optional[UUID]] = mapped_column(
        UUID(as_uuid=True),
        nullable=True,
    )

    viewer_type: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
    )
    ...
    customer: Mapped[Optional["Customer"]] = relationship(
        back_populates="service_views"
    )

    __table_args__ = (
        CheckConstraint(
            "viewer_type IN ('customer', 'anonymous')",
            name="chk_service_viewer_type",
        ),

        CheckConstraint(
            """
            (viewer_type = 'customer' AND customer_id IS NOT NULL)
            OR
            (viewer_type = 'anonymous' AND visitor_id IS NOT NULL)
            """,
            name="chk_service_view_identity",
        ),

        Index(
            "idx_service_views_service_viewed_at",
            "service_id",
            "viewed_at",
        ),

        Index(
            "idx_service_views_customer_viewed_at",
            "customer_id",
            "viewed_at",
        ),

        Index(
            "idx_service_views_visitor_viewed_at",
            "visitor_id",
            "viewed_at",
        ),
    )