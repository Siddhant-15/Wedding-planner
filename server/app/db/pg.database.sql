...
CREATE TABLE service_views (
    ...
    -- UUID generated and persisted by the browser for guests
    visitor_id      UUID,

    viewer_type     VARCHAR(20) NOT NULL
        CHECK (viewer_type IN ('customer', 'anonymous')),

    viewed_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    CONSTRAINT chk_service_view_identity CHECK (
...
-- Main query: service analytics
CREATE INDEX idx_service_views_service_viewed_at
    ON service_views(service_id, viewed_at DESC);

-- Logged-in customer analytics
CREATE INDEX idx_service_views_customer
    ON service_views(customer_id, viewed_at DESC)
    WHERE customer_id IS NOT NULL;

-- Anonymous visitor deduplication
CREATE INDEX idx_service_views_visitor
    ON service_views(visitor_id, viewed_at DESC)
...
DO $$
BEGIN
    CREATE TYPE review_section AS ENUM (
        'basic_information',
        'location',
        'media',
        'pricing_variants',
...
CREATE TABLE IF NOT EXISTS service_version_review_items (
    ...
    CONSTRAINT uq_review_item_version_section
        UNIQUE (service_version_id, section)
);
...
CREATE INDEX IF NOT EXISTS idx_review_item_version
ON service_version_review_items(service_version_id);


CREATE INDEX IF NOT EXISTS idx_review_item_status
ON service_version_review_items(status);
